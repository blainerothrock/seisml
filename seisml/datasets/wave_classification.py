...
@gin.configurable()
def wave_classification_split(training_split=0.7, batch_size=128, shuffle=True):
    """
    ...
    dw_size = len(dw)
    indices = list(range(dw_size))
    offset = int(np.floor(training_split * dw_size))
...
class DetectedWave(Dataset):
    ...
    def __init__(
            self,
            data_dir=os.path.expanduser('~/.seisml/data/wave_data.hdf5'),
            force_download=False,
            downloadable_data=DownloadableData.SCSN_WAVE_SAMPLE):

        # download data if it does not exist in path
