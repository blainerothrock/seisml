...
def test_whiten_mean_loss():
    ...
    l = WhitenedKMeansLoss()

    _loss_a = l(labels, labels, weights).item()
    _loss_b = l(embedding, labels, weights).item()
    assert _loss_a < _loss_b, 'identity loss should be less'